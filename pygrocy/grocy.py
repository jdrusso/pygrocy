...
class Grocy(object):
    ...
    def to_defrost(
        ...
        raw_meal_plan = self._api_client.get_meal_plan(query_filters)[0]
        _LOGGER.debug(f"Got {raw_meal_plan}")
        ...
        _LOGGER.debug(f"Submitting recipe fulfillment for {recipe_id}")
        ...
        _LOGGER.debug(f"Recipe ingredient names are {recipe_ingredient_names}")
        ...
        _LOGGER.debug(f"Stock in freezer is {stock_in_freezer}")
        ...
        _LOGGER.debug(f"Products got {len(products)} responses, first it {products[0]}")
        ...
        _LOGGER.debug(f"Products to defrost are {products_to_defrost}")
